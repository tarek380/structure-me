...
// ------------------------------------------------------------------
// HTML escape — for plain text fields going into HTML attributes
// ------------------------------------------------------------------
...
async function main() {
  ...
  for (const slug of SLUGS) {
    ...
    let html = renderServicePage(originalHtml, doc, slug)

    // ── SEO injection ─────────────────────────────────────────────────────────────
    // Fallback OG image: use hero image path if no Sanity ogImage asset set
    const fallbackOg = doc.hero?.imageAsset
      ? null
      : doc.hero?.heroImage
        ? `https://www.structureme.com.au/${doc.hero.heroImage}`
        : null
    html = injectSeo(html, doc, fallbackOg)
